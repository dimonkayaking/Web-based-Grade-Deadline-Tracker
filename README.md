...
- [6. Особенности реализации](#6-особенности-реализации)
  ...
  - [6.4. План миграции данных](#64-план-миграции-данных)
  - [6.5. Форматы экспорта и импорта данных](#65-форматы-экспорта-и-импорта-данных)
...
|-----------|------------|
| Браузеры | Chrome 90+, Yandex 22+, Edge 90+ |
...
#### 6.4. План миграции данных (при интеграции с внешними системами)
...
#### 6.5. Форматы экспорта и импорта данных

Система должна поддерживать экспорт и импорт данных в следующих форматах для обеспечения гибкости работы с данными:

**6.5.1. Поддерживаемые форматы**

| Формат | Расширение | Назначение | Поддержка |
|--------|------------|------------|-----------|
| **CSV** | `.csv` | Массовый импорт/экспорт данных, открывается в Excel, компактный размер | ✅ Обязательный |
| **XLSX** | `.xlsx` | Экспорт отчетов для пользователей с поддержкой форматирования и нескольких листов | ✅ Обязательный |

**6.5.2. CSV**
- **Назначение:** массовый импорт/экспорт табличных данных, открывается в Excel
- **Разделитель:** запятая (`,`) или точка с запятой (`;`) в зависимости от локали
- **Кодировка:** UTF-8
- **Особенности:** каждая таблица экспортируется в отдельный CSV-файл
- **Пример (grades.csv):**
```csv
grade_id,subject_id,grade_value,graded_at,description
1,1,5,2026-06-15,Отличный ответ на семинаре
2,1,4,2026-06-20,Допущена небольшая ошибка
3,2,Зачет,2026-06-22,Лабораторная работа защищена
```

**6.5.3. XLSX (Excel)**
- **Назначение:** экспорт форматированных отчетов для пользователей с возможностью открытия в Excel
...
**6.5.4. Рекомендации по использованию**

| Сценарий | Рекомендуемый формат | Обоснование |
|----------|---------------------|-------------|
| Массовый импорт данных | CSV | Быстро, компактно, открывается в Excel |
...
| Импорт из внешних систем (Дневник.ру, БРС) | CSV | Универсальный формат для обмена |
...
│   └── utils/               # Утилиты
│       ├── validators.py
│       └── export_import.py # Экспорт/импорт CSV/XLSX
...
- Включает все эндпоинты:
  ...
  - Экспорт/импорт данных (CSV, XLSX)
