...
- [2. Функциональные требования](#2-функциональные-требования)
  ...
  - [2.2. Необязательные функции по желанию команды](#22-необязательные-функции-по-желанию-команды)
...
- [4. Стек технологий](#4-стек-технологий)
  ...
  - [4.4. Архитектурное расширение (внедрение «1С» по желанию команды)](#44-архитектурное-расширение-внедрение-1с-по-желанию-команды)
...
- [6. Особенности реализации](#6-особенности-реализации)
  ...
  - [6.4. Экспорт, импорт и миграция данных](#64-экспорт-импорт-и-миграция-данных)
...
| CSV | Comma-Separated Values — табличный формат с разделителями |
...
| **Бизнес-логика** | Загрузка всех предметов пользователя, клик по строке → переход на страницу деталей предмета (с оценками). Таблица со столбцами: Предмет, Оценка - средний балл, Преподаватель, Дата окончания |
...
| Поле | Описание |
|------|----------|
| **Входные данные** | Те же поля, что и у создания (название, преподаватель, цвет, дата окончания) |
...
#### 2.2. Необязательные функции по желанию команды
...
**Ссылка на макет:** [https://www.figma.com/design/Pg5qkXGw5wOGdawA5PPNid/%D0%92%D0%B5%D0%B1-%D1%82%D1%80%D0%B5%D0%BA%D0%B5%D1%80-%D1%83%D1%81%D0%BF%D0%B5%D0%B2%D0%B0%D0%B5%D0%BC%D0%BE%D1%81%D1%82%D0%B8-%D0%B8-%D0%B4%D0%BE%D0%BB%D0%B3%D0%BE%D0%B2?node-id=0-1&t=fKSefGJK5I6oEkTe-1](https://www.figma.com/design/Pg5qkXGw5wOGdawA5PPNid/%D0%92%D0%B5%D0%B1-%D1%82%D1%80%D0%B5%D0%BA%D0%B5%D1%80-%D1%83%D1%81%D0%BF%D0%B5%D0%B2%D0%B0%D0%B5%D0%BC%D0%BE%D1%81%D1%82%D0%B8-%D0%B8-%D0%B4%D0%BE%D0%BB%D0%B3%D0%BE%D0%B2?node-id=0-1&t=fKSefGJK5I6oEkTe-1)

3.1. Регистрация/авторизация

3.2. Боковая панель навигации (логотип, кнопки "Предметы", "Календарь", смена темы, "Выход")

3.3. Страница предметов (таблица с колонками: Предмет, Оценка - средний балл, Преподаватель, Дата окончания)

3.4. Создание/редактирование/удаление предмета

3.5. Страница оценок предмета (таблица: Значение, Описание, Дата получения)

3.6. Создание/редактирование оценки

3.7. Страница календаря (календарная сетка с заданиями-дедлайнами, подсвеченными цветом предмета)

3.8. Создание/редактирование/удаление задания

3.9. Состояния интерфейса
 - При отсутствии данных в таблицах выводится надпись: «Добавьте новый предмет/новую оценку»
 - При долгом импорте выводится надпись: «Идёт загрузка» или «Загрузка»
 - При долгом экспорте выводится надпись: «Подготовка данных»
 - При ошибке во время действия выводим надпись: «Не удалось выполнить действие»
 - При другом типе ошибок выводим: «Непредвиденная ошибка»
...
#### 4.4. Архитектурное расширение (внедрение «1С») по желанию команды
...
#### 6.4. Экспорт, импорт и миграция данных

Система должна поддерживать экспорт, импорт и миграцию данных для обеспечения гибкости работы пользователей с данными.

**6.4.1. Поддерживаемые форматы**

| Операция | Формат | Поддержка |
|----------|--------|-----------|
| **Экспорт** | XLSX, CSV | ✅ |
| **Импорт** | XLSX, CSV | ✅ |
...
**6.4.3. CSV**
- **Назначение:** экспорт и импорт табличных данных, открывается в Excel, компактный размер
- **Разделитель:** запятая (`,`) или точка с запятой (`;`) в зависимости от локали
- **Кодировка:** UTF-8
- **Особенности:** каждая таблица экспортируется/импортируется из отдельного CSV-файла

**6.4.4. Поддерживаемые сущности для миграции**

| Сущность | Экспорт | Импорт |
|----------|:-------:|:------:|
| Предметы | ✅ (XLSX, CSV) | ✅ (XLSX, CSV) |
| Оценки предмета | ✅ (XLSX, CSV) | ✅ (XLSX, CSV) |

**6.4.5. Сценарии использования**

| Сценарий | Описание |
|----------|----------|
| Экспорт в XLSX | Пользователь выгружает свои предметы и оценки в XLSX-файл |
| Экспорт в CSV | Пользователь выгружает свои предметы и оценки в CSV-файл |
| Импорт из XLSX | Пользователь загружает ранее выгруженный XLSX-файл для восстановления данных |
| Импорт из CSV | Пользователь загружает CSV-файл (например, выгруженный из внешней системы или созданный вручную) |

**6.4.6. Обработка ошибок при импорте**

| Случай | Обработка |
|--------|-----------|
| Дубликаты записей | Обновление существующих записей |
| Ошибки в файле | Логирование с указанием строки и причины |
| Отсутствие обязательных полей | Импорт отклоняется, пользователь получает сообщение об ошибке |
| Оценка привязана к несуществующему предмету | Оценка не импортируется, пользователь получает уведомление |

**6.4.7. Рекомендации по использованию**

| Сценарий | Формат | Обоснование |
|----------|--------|-------------|
| Экспорт данных пользователю | XLSX, CSV | Удобно для Excel, поддержка форматирования |
| Импорт данных в систему | XLSX, CSV | Гибкость: XLSX — для форматированных данных, CSV — для быстрого импорта |
| Отчеты с форматированием | XLSX | Поддержка стилей, нескольких листов |
| Импорт из внешних систем | CSV | Универсальный формат для обмена |
...
│   └── utils/               # Утилиты
│       ├── validators.py
│       └── export_import.py # Экспорт/импорт XLSX и CSV
...
Пример спецификации находится в файле `swagger.yaml`
